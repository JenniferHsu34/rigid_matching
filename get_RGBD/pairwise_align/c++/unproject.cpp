...
void mexFunction(
     int nargout,                //shape.vertexPoss
     mxArray *output[],          //shape.faceVIds
     int nargin,                 //lookat
     const mxArray *input[]) {   //points
  /* check argument */
  if (nargin != 4) {
  ...
  output[0] = mxCreateDoubleMatrix(5, numRays, mxREAL);
  data = mxGetPr(output[0]);
  
  //for (unsigned rayId = 0; rayId < 1; ++rayId) { 
  for (unsigned rayId = 0; rayId < numRays; ++rayId) {
	  ...
	  data[5*rayId] = pt.faceId + 1;
	  data[5*rayId+1] = pt.paraU;
	  data[5*rayId+2] = pt.paraV;
	  data[5*rayId+3] = pt.distance; 
          data[5*rayId+4] = pt.visible; 
         /* data[4*rayId] =  1;
	  data[4*rayId+1] = 1;
	  data[4*rayId+2] = 1;
	  data[4*rayId+3] = 1; */
