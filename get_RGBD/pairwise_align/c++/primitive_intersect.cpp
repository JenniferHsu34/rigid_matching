...
bool RayTriangleIntersection(const Ray3D &ray,
  ...
  double det = edge1*pvec;
 //if feterminant is near zero, ray lies in plane of triangle or ray is parallel to plan of triangle
 ...
 //angle between ray and the triangle n
  Vector3d normal = edge1.crossProduct(edge2);
  float normRay=sqrt(ray.direction[0]*ray.direction[0]+ray.direction[1]*ray.direction[1]+ray.direction[2]*ray.direction[2]);
  float normNormal=sqrt(normal[0]*normal[0]+normal[1]*normal[1]+normal[2]*normal[2]);
  //cosine value between the normal of the triangle and ray direction
  double cosine=cos(edge1*ray.direction/normRay/normNormal);
 
  //std::cout<<"cosine: "<< cosine <<std::endl;

  pt->visible = fabs(cosine);
